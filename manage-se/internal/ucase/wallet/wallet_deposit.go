...
func (c deposit) Serve(data *appctx.Data) appctx.Response {
	ctx := tracer.SpanStart(data.Request.Context(), "usecase.deposit")
	defer tracer.SpanFinish(ctx)

	responder := appctx.NewResponse().WithState("deposit")
	...
	wallet, err := c.service.Deposit(ctx, input)
	if err != nil {
...
func NewDeposit(service wallet.Wallet) contract.UseCase {
	return &deposit{service: service}
}
