...
func (c enabled) Serve(data *appctx.Data) appctx.Response {
	ctx := tracer.SpanStart(data.Request.Context(), "usecase.enabled")
	defer tracer.SpanFinish(ctx)

	responder := appctx.NewResponse().WithState("enabled")

	wallet, err := c.service.EnableWallet(ctx)
	if err != nil {
...
func NewEnabled(service wallet.Wallet) contract.UseCase {
