...
func (db *dbMulti) BeginTx(ctx context.Context, opts *sql.TxOptions) (*sqlx.Tx, error) {
	return db.dbWrite.BeginTx(ctx, opts)
}

func (db *dbMulti) ExecTx(ctx context.Context, tx *sqlx.Tx, query string, args ...any) (sql.Result, error) {
	return db.dbWrite.ExecTx(ctx, tx, query, args...)
}

func (db *dbMulti) QueryTx(ctx context.Context, tx *sqlx.Tx, query string, args ...interface{}) (*sql.Rows, error) {
	return db.dbWrite.QueryTx(ctx, tx, query, args...)
}

func (db *dbMulti) CommitTx(ctx context.Context, tx *sqlx.Tx) error {
	return db.dbWrite.CommitTx(ctx, tx)
}

func (db *dbMulti) RollbackTx(ctx context.Context, tx *sqlx.Tx) error {
	return db.dbWrite.RollbackTx(ctx, tx)
}
