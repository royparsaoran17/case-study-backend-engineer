...
func (c customer) FindCustomerByPhonePassword(ctx context.Context, phone, password string) (*entity.CustomerDetail, error) {
	query := `SELECT 
        jsonb_build_object(
            'id', c.id,
            'name', c.name,
	...
	var b []byte
	err := c.db.QueryRow(ctx, query, phone, password).Scan(&b)
	if err != nil {
